...
def f1_score_multiclass(labels: torch.Tensor, raw_predictions: torch.Tensor, average: str = 'macro') -> float:
    precision = precision_score_multiclass(labels, raw_predictions, average)
    recall = recall_score_multiclass(labels, raw_predictions, average)
    return 2 * (precision * recall) / (precision + recall) if precision + recall > 0 else 0.0


def f1_score_binary(labels: torch.Tensor, raw_predictions: torch.Tensor) -> float:
    precision = precision_score_binary(labels, raw_predictions)
    recall = recall_score_binary(labels, raw_predictions)
    return 2 * (precision * recall) / (precision + recall) if precision + recall > 0 else 0.0
